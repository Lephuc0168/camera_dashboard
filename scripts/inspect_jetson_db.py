...
def run_inspection():
    ...
    print(f[*] Connection: {connected_url.split('@')[-1] if '@' in connected_url else connected_url}\n)
    ...
    for t in spec_tables:
        if t in tables:
            cur.execute(fSELECT COUNT(*) as cnt FROM {t};)
    ...
    if 'persons' in tables:
        cur.execute("
            SELECT p.person_id, p.full_name, p.student_code, p.status, COUNT(f.embedding_id) as emb_count
            FROM persons p
            LEFT JOIN face_embeddings f ON p.person_id = f.person_id
            GROUP BY p.person_id, p.full_name, p.student_code, p.status
        ...
        for p in persons:
            ...
            print(f • {p['full_name']:<22} | Mã: {code:<12} | Status: {p['status']} | Vectors: {p['emb_count']})
        print()
